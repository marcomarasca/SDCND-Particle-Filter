    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          auto avg_pred = t_pred / i;
          auto avg_update = t_update / i;
          auto avg_resample = t_resample / i;

          chrono::duration<int64_t, nano> total = t_pred + t_update + t_resample;

          cout << setw(10) << "Total (ms)" << setw(10) << "Pred (ns)" << setw(12) << "Update (ns)" << setw(14)
               << "Resample (ns)" << endl;
          cout << setw(10) << chrono::duration_cast<chrono::milliseconds>(total).count() << setw(10) << avg_pred.count()
               << setw(12) << avg_update.count() << setw(14) << avg_resample.count() << endl;
